...
```{r babynames-over-time, echo = FALSE, out.width = "95%"}
library(babynames)
library(hrbrthemes)
library(magick)
...
# Plot females
p_female <- babynames_top5 %>%
  filter(sex == "F") %>%
  mutate(name = fct_reorder2(.f = name, .x = year, .y = n)) %>%
  ggplot(mapping = aes(x = year, y = n, group = name, color = name)) +
  geom_line() +
  geom_point() +
  ...
  labs(
    ...
    subtitle = "Most popular female names in 2020",
    x = "Year",
...
f_gif <- animate(p_female)

p_male <- babynames_top5 %>%
  filter(sex == "M") %>%
  mutate(name = fct_reorder2(.f = name, .x = year, .y = n)) %>%
  ggplot(mapping = aes(x = year, y = n, group = name, color = name)) +
  geom_line() +
  ...
  labs(
    title = "Baby name popularity in the United States",
    subtitle = "Most popular male names in 2020",
    x = "Year",
    y = "Number of babies born",
    color = NULL,
    caption = "Source: Social Security Administration and the babynames package"
  ...
  theme_ipsum(base_size = 18) +
  transition_reveal(year)
m_gif <- animate(p_male)

# combine into a single plot for the slides
# faceting doesn't work due to the use of color
a_mgif <- image_read(f_gif)
b_mgif <- image_read(m_gif)
